...
class MovingBox:
    ...
    def add_item(self, item_name: str, item_count: int):
        ...
        :return: None
        """
        self._boxes[self._name]=[self._items.append(item_name) for _ in range(item_count)]


    def remove_item(self, item_name: str, item_count: int):

        self._boxes[self._name] = [self._items.remove(item_name) for _ in range(item_count)]
