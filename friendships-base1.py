...
def read_friendship_network(filename):
    ...
    # initialize empty dictionary <network>
    network = {}
...
def add_friendship(network, name1, name2):
    ...
    :return: network, dictionary containing friends network
    """
    ...
    #print('name1',name1)
    #print('name2',name2)
    # checking to make sure no one can be added as his/her own friend
    if name1 == name2:
        return

    # check if <name1> is in network or not, if not then create empty list
    if name1 not in network:
        network[name1] = []

    # check if <name2> is in network or not, if not then create empty list
    if name2 not in network:
        network[name2] = []

    # check if <name2> is in nested list as value of <name1> or not
    # if not then append it to the list
    if name2 not in network[name1]:
        network[name1].append(name2)

    # check if <name1> is in nested list as value of <name2> or not
    # if not then append it to the list
...
def print_command(network, namelist):
    ...
    # print the information of all friendships in alphabetical order
    for friendsK in sorted(network):
        print(friendsK)
        for friends in sorted(network[friendsK]):
            print(f"- {friends}")
...
def friends_command(network, namelist):
    ...
    # print name's friend in alphabetical order from network dictionary
    for friend in sorted(network[name]):#change
        print(friend)
def common_friends_command(network, namelist):
    ...
    if len(commonfriends) == 0:
        print(f"{name1} and {name2} have no common friends.")
          ...
          # print the common friends in alphabetical order
    for names_of_friend in sorted(commonfriends):
             print(names_of_friend)
def main():
    ##################################
    #  ÄLÄ MUUTA TÄTÄ FUNKTIOTA!     #
    #  DO NOT MODIFY THIS FUNCTION!  #
