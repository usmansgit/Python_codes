...
class Car:
    ...
    def __init__(self, tank_size, gas_consumption):
        ...
        self.__fuel=0.0
        self.__kilometers=0.0

    def print_information(self):
        print(f"The tank contains {self.__fuel} liters of gas and the odometer shows {self.__kilometers} kilometers.")

    def to_fill(self, liters):
        if liters < 0:
            print("You cannot remove gas from the tank")
        elif self.__fuel + liters > self.__tank_volume:
            self.__fuel= self.__tank_volume
    ...
    def drive(self, kilometers):
        ...
        elif kilometers > self.__fuel*self.__consumption:
            self.__kilometers=self.__kilometers+(self.__fuel*self.__consumption)
            self.__fuel = self.__fuel - self.__tank_volume
        elif kilometers < self.__fuel*self.__consumption:
            self.__kilometers=self.__kilometers+kilometers
            self.__fuel = self.__fuel - (kilometers / self.__consumption)
    # TODO:
