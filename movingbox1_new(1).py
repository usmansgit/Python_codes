...
class MovingBox:
    ...
    def __init__(self, box_name):
        """
        Creates a new object for the Moving box class with an empty list of items in the box
    ...
    def add_item(self, item_name: str, item_count: int):
        """
        Add item to the box. Appends the item to the list
        `item_count` number of times
        :param item_name: name of the item to add
        :param item_count: count of items in the box
        :return: None
        """
        updated = False
        if self.box_name in self._boxes:
            for key, value in self._boxes.items(): 
                for item in value:
                    for index, value in enumerate(item):
                        if (item[0] == item_name):
                            update_item = self._boxes[self.box_name][index]
                            update_item.extend([item_name] * item_count)
                            updated = True
                            break
            if (not updated):
                self._boxes[self.box_name].append([item_name] * item_count)
            
        else:
            self._boxes[self.box_name] = [[item_name] * item_count]


    def get_total_items(boxes):
        print(boxes)
        total_item = 0
    ...
    def get_total_items_list(box):
        total_item = 0
        for value in box: 
            total_item += len(value)
        
        return total_item

    def  list_box_content(self, all_boxes: dict, box_names: list):         #list all

        total = MovingBox.get_total_items_list(self._boxes[self.box_name])
        print("this")
        print('Box "' + self.box_name +  '" contains ' + str(total) + 'items.')
        for item in self._boxes[self.box_name]:
            item.sort()
            print(' ',str(len(item)), item[0])


    def list_content(self):           #list specific box content
        for key, value in self._boxes.items():
            total = MovingBox.get_total_items(self._boxes)
            print(total)
            if total!=0:
                print('Box "' + key + '" contains ' + str(total) + ' items.')
                for item in value:
                    value.sort()
                    print(' ',str(len(item)), item[0])
            else:
                print('Box "' + key + '" contains 0 items.')
